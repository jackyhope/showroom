...
job_save_url = 'http://*******/crawler/resume/uploadSourceUrl'  # 内网
# job_save_url = 'http://*******/crawler/resume/uploadSourceUrl'    #外网
...
sock.bind(('192.168.1.149', 8000))  # 绑定主机ip和端口号
...
yourAccessKeyId = 'LTAIXGkMmsFY5cVS'
yourAccessKeySecret = 'LZscNlVIGmmBzuWEatfX6HBvCHP4f7'
...
# 图片转png
def image2png(inputFile, outputFile, saveq, get_data_1):
    try:
        try:
            try:
...
def pdf2img(source_path, target_path):
    ...
    return totaling
def pdftopng(inputFile, outputFile, get_data_1):
    ...
    png_from = outputFile + '/result.jpg'
    png_to = outputFile + '/result.png'
    image2png(png_from, png_to, 1, get_data_1)
def wordtopng(inputFile, outputFile, get_data_1):
    word2pdf(inputFile, outputFile)
    outfile = str(now_time)
    pdftopng(outputFile, outfile, get_data_1)
...
while True:
    ...
    try:
        ...
        outfile = str(now_time) + '/' + str(now_time) + '.png'
        ...
        if threading.activeCount() < 3:
            ...
            if '.pdf' in input_file:
                outfile = str(now_time)
                th_1 = threading.Thread(target=pdftopng, args=(input_file, outfile, get_data))
                th_1.start()
            elif '.doc' in input_file or '.docx' in input_file:
                path_1 = os.getcwd()
                th_1 = threading.Thread(target=wordtopng, args=(
                path_1 + '\\' + str(now_time) + '\\' + str(now_time) + '.' + input_file.split('.')[-1],
                path_1 + '\\' + str(now_time) + '\\' + str(now_time) + '.pdf', get_data))
                th_1.start()
            else:
                th_2 = threading.Thread(target=image2png, args=(input_file, outfile, 1, get_data))
                th_2.start()
